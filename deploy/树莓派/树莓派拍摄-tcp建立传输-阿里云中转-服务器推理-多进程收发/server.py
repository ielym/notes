...
def process_pair(client_conn, inference_conn):
    ...
    queue_error = multiprocessing.Queue()

    thread_client = multiprocessing.Process(target=recv_from_client, args=(client_conn, queue_data, queue_error))
    thread_server = multiprocessing.Process(target=send_to_inference, args=(inference_conn, queue_data, queue_error))
