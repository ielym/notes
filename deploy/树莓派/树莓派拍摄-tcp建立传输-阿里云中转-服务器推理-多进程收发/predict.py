...
def preprocess(ori_img, input_size):
    ...
    padding_img = np.ones([input_size, input_size, 3], dtype=np.float32) * 0
...
class Predict():
    def __init__(self):
        self.ort_session = onnxruntime.InferenceSession("u_yolov3.onnx", providers=['CUDAExecutionProvider'])
        ...
        data_dict = yaml.safe_load(open('./coco.yaml', 'r'))
        category_names = data_dict['categories']
        self.category_id_dict = {idx:name for idx, name in enumerate(category_names)}

    def __call__(self, frame):


        image, ori_img, resize_ratio, padding_left, padding_top, ori_height, ori_width = preprocess(frame, self.input_size)
        ...
        scores, categories, boxes = postProcess(ort_outs, strides, self.anchors, conf_thres, iou_thres, resize_ratio, padding_left, padding_top, ori_width, ori_height)

        results = []
        categories_unique = set()
        for box, category, score in zip(boxes.astype(np.int32), categories.astype(np.int32), scores.astype(np.float32)):
            xmin, ymin, xmax, ymax = box
            categories_unique.add(int(category))
            results.append((xmin, ymin, xmax, ymax, self.category_id_dict[int(category)], int(category), float(score)))

        return results, categories_unique