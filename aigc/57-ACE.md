...
![image-20241117140813112](imgs/57-ACE/image-20241117140813112.png)

- 与StyleBooth的构建成对数据方法类似，我们构建了包含80种风格的6.3w对风格化训练数据。
- 此外，还搜集了一些真实的风格化数据，以及生成了一些风格化数据。这些风格化数据作为target，并生成对应电影风格或真实风格的数据作为source。

- 通过这种方式一共构建了7w对数据，包含400种高质量的风格。

---

为了提升数据质量：

- 与StyleBooth类似，使用CLIP score来过滤掉差异太小或太大的图像对。
- 使用CSR [Measuring Style Similarity in Diffusion Models] 在每个风格内部进行风格聚类，阈值为0.65，使用余弦相似度

### 3.3.5 通用编辑

![image-20241117142141848](imgs/57-ACE/image-20241117142141848.png)

- 需要一张源图和caption。
- 构造方法包含两种：
  - 基于聚类的方法：
    - 使用Siglip提取embedding，使用K-means聚类
    - 之后使用face similarity或object consistency等方法来进行细粒度的聚类
    - 最后，使用一个二分类器来判断内容相关性。
  - 基于生成的方法：
    - 使用IP-Adapter生成，权重0.6。
    - 之后使用二分类器来过滤内容不相关的数据对。

### 3.3.6 元素编辑

- **文字编辑：**

  ![image-20241117150250856](imgs/57-ACE/image-20241117150250856.png)

  ​	![image-20241117145344770](imgs/57-ACE/image-20241117145344770.png)

  - “文本” 指的是真的文字。文字移除和添加文字是互逆的。

  - **文字移除：** 

    - 使用开源的文字数据集 AnyWord3M，LaionGlyph10M
    - 首先Mask掉所有文字区域
    - 之后使用Inpainting方法重绘文字区域。为了保证重绘之后的图像不包含任何文本，使用OCR进行检测（如，PP-OCR），并移除含有文字的图像。
    - 最后，使用人工标注的数据训练了一个小的图像质量预测模型，根据分数进行过滤。

  - **文字渲染：**

    - 对于任意的图像数据集，首先使用OCR检测，确保图像种不包含任何文字。
    - 之后使用如 AnyText等模型，随机的加入文字。加入的位置也是随机的。包含中英文文字。
    - 最后使用OCR检测，保证渲染的文字是正确的。

  - 训练时，文字移除和文字渲染的数据会合并到一起进行训练。

  - 构造指令时，在指令种加入位置和颜色描述。

    ![image-20241117150137135](imgs/57-ACE/image-20241117150137135.png)

- **物体编辑**

  ![image-20241117153232989](imgs/57-ACE/image-20241117153232989.png)

  - 用RAM识别主题，用Grounded-SAM分割物体
  - 根据面积等删除不合理的masks
  - 使用LaMa进行inpainting
  - 使用二分类器进行质量筛选
...
![image-20241117153653846](imgs/57-ACE/image-20241117153653846.png)

- **Unconditional Inpainting：**
  - 使用LaMa构造数据，mask随机使用bbox或不规则的形状
- **Text-Guided Inpainting**
  - 从图像种提取mask
  - 对每个mask进行详细的文本描述
  - 最后构建三元组：源图，局部mask，局部物体描述
- **OutPainting**
  - 对需要扩展的区域和方向进行随机mask处理，以适应不同的图像补全场景
